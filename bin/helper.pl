...
my $helper = RDFMapperUtils::Interactive::Helper::getInstance(config_file => $config_file);

if (!defined($helper)){
    $logger->logdie("Could not instantiate RDFMapperUtils::Interactive::Helper");
}

if (defined($mapper_config_file)){
    $helper->setMapperConfigFile($mapper_config_file);
}

if (defined($outdir)){
    $helper->setOutdir($outdir);
}

if (defined($outfile)){
    $helper->setOutfile($outfile);
}
...
if (defined($infile)){
    $helper->setInfile($infile);
}

if (defined($infile_type)){
    $helper->setInfileType($infile_type);
}
...
sub checkCommandLineArguments {
    ...
    if (!defined($outdir)){
        ...
        $outdir = DEFAULT_OUTDIR_BASE . File::Basename::basename($0) . '/' . time();
